...
function Base:get_active_or_create()
...
function Base:toggle()
  local active = self:get_active_or_create()
...
function Base:append_text(bufnr, start_line, end_line)
  ...
  vim.api.nvim_buf_set_lines(self.prompt.bufnr, line_index, line_index, false, contents)
end
