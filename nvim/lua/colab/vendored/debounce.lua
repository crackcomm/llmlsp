-- Credit: telescope.nvim -> debounce.lua
...
function M.throttle_leading(fn, timeout)
...
---@param fn (function) Function to throttle
---@param timeout (number) Timeout in ms
---@param last? (boolean) Whether to use the arguments of the last
---call to `fn` within the timeframe. Default: Use arguments of the first call.
---@returns (function, timer) Throttled function and timer. Remember to call
---`timer:close()` at the end or you will leak memory!
function M.throttle_trailing(fn, timeout, last)
  td_validate(fn, timeout)
  local timer = vim.loop.new_timer()
  local running = false
...
---@param timeout? (number) Timeout in ms, default 2000.
---@param firstlast? (boolean) Whether to use the 'other' fn call
---strategy.
function M.test_defer(bouncer, timeout, firstlast)
  local bouncers = {
